...
class MaterialAwareConverter {
  ...
  private extractMaterialData(material: Material): MaterialData {
    if (material instanceof MeshStandardMaterial) {
      return {
        ...
        side: material.side === 2 ? 'double' : 
              material.side === 1 ? 'back' : 'front'
      };
    }
  ...
  private async loadGLTFModel(filePath: string): Promise<{ geometry: BufferGeometry; materials: MaterialData[] }> {
  ...
  private async encodeGeometry(geometry: BufferGeometry): Promise<ArrayBuffer> {
    ...
    const vertices = geometry.attributes.position?.array || new Float32Array([0, 0, 0, 1, 0, 0, 0, 1, 0]);
  ...
  public async convertToBinary(inputPath: string, outputPath: string): Promise<void> {
    try {
      ...
      materials.forEach((mat, index) => {
        console.log(`  Material ${index + 1}: ${mat.name || 'unnamed'} (color: #${mat.color?.toString(16).padStart(6, '0')})`);
      });
...
if (require.main === module) {
  ...
  if (command === 'sample') {
    ...
    converter.convertToBinary(inputPath, output)
    ...
    console.log('  node material-aware-converter.js convert <input> [output] - Convert model to binary');
  }
}
